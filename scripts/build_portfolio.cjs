...
const wallpaperRetouch = 'Patterned wallpaper replaced with a neutral wall using AI assistance for privacy.';
const hasWallpaperRetouch = p => ['breadboard-prototype', 'home-assistant-dashboard'].includes(p.id);
const graph = [
...
<link rel="canonical" href="${data.canonical}"><link rel="icon" href="assets/favicon.svg" type="image/svg+xml"><link rel="stylesheet" href="styles.css?v=20260907-bold-cv"><link rel="stylesheet" href="portfolio.css?v=20260907-full-row-photo">
...
<div class="portfolio-toolbar"><a href="/">← Back to CV</a><nav aria-label="Profile links"><a href="https://github.com/pilprod">GitHub</a><a href="${data.linkedin.profile}">LinkedIn</a><details class="language-menu"><summary aria-label="Choose language, current: English"><span class="current-language-flag" aria-hidden="true">🇬🇧</span><span class="language-caret" aria-hidden="true">▾</span></summary><div class="language-options"><a href="/portfolio.html" lang="en" aria-current="page"><span aria-hidden="true">🇬🇧</span> English</a><a href="/ru/portfolio.html" lang="ru"><span aria-hidden="true">🇷🇺</span> Русский</a></div></details></nav></div>
...
for(const p of data.projects){
  ...
  if(p.id==='home'){
    markdown+='\n### Lab photographs\n\nThese show the wider historical installation, not build verification or all code in either public repository. Both aeroponics READMEs contain the gallery. Seven photos have background or identifying-area AI retouching. The root-chamber photograph has not been redrawn.\n\n';
    for(const photo of data.photos)markdown+=`- [${photo.title}](https://papou.work/${photo.file}): ${photo.caption}${photo.retouched?' '+(hasWallpaperRetouch(photo)?wallpaperRetouch:'AI-retouched background or identifying areas.'):''} [Published source](${photo.source}).\n`;
  }
  markdown+='\n';
}
