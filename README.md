...
- `portfolio-images/` contains the same privacy-reviewed JPEGs as the two aeroponics READMEs. Six images disclose AI background/identifying-area retouching. One root-chamber photograph shows both the roots and internal tubing, without repetitive views. The diagram and root evidence are not AI-redrawn. Do not replace these files with the unreviewed originals or claim a photography license that has not been granted.
- The breadboard photograph uses a 1024 × 768 privacy-edited version of the owner's supplied photo. Patterned wallpaper was replaced with a neutral wall using AI assistance; this is background retouching, not a claim of pixel-identical image preservation. The gallery and discovery data use `breadboard-prototype-privacy-20260909.jpg` to avoid the previous image cache. The old `breadboard-prototype-1024.jpg` URL remains a compatibility alias containing exactly the same edited bytes. Display scales proportionally, and image links open the complete frame.
