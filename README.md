...
The featured R&D heading is now exactly `Agent Orchestration Infrastructure` on mobile, tablet, desktop and print/PDF, following the user's naming correction. It has one shared heading and accessible name rather than separate mobile and desktop variants. The selected-code link named YourOwn.Chat remains unchanged.
